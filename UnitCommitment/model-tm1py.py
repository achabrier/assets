...
## establish PA credentials
pTenant='ctpeu'
pServer='UnitCommitment'
...
def getCubeView(cube_name, view_name):
    with TM1Service(base_url=base_url,
       ...
       df = Utils.build_pandas_dataframe_from_cellset(raw_data, multiindex=True)
       df.reset_index(inplace=True)
       df = df[df['Version']==scenario]
       df.drop('Version', inplace=True, axis=1)
       df.rename(columns={'Values': scenario}, inplace=True)
       print (df)
       return df
...
inputs["Units"] = getCubeView("Units", "Units")
inputs["Loads"] = getCubeView("Loads", "Loads")
inputs["UnitMaintenances"] = getCubeView("UnitMaintenances", "UnitMaintenances")
inputs["Weights"] = getCubeView("Weights", "Weights")
...
df_exchanges = pd.DataFrame(columns=['Periods', 'Max', 'Price'],
                                 data=[[p, 0, 0] for p in periods])
...
if ucpm.solve(log_output=True):
    ...
    df_plan = pd.DataFrame(columns=['Version', 'Units', 'Periods', 'production', 'started', 'used', 'fixed_cost', 'variable_cost', 'startup_cost', 'co2_cost'],
                           data=[[scenario, u, p, production[u,p].solution_value, turn_on[u, p].solution_value, in_use[u,p].solution_value,
                            in_use[u,p].solution_value * df_units.value[u,"constant_cost"],
                            production[u,p].solution_value * df_units.value[u,"linear_cost"],
                            turn_on[u,p].solution_value * df_units.value[u,"start_up_cost"],
                            production[u,p].solution_value * df_units.value[u,"co2_cost"]] for u in units for p in periods])

    outputs = {}
    outputs['exchanged'] = df_exchanged
    print (df_exchanged)
    outputs['plan'] = df_plan
else:
    print ("  Infeasible ")
    outputs = {}
